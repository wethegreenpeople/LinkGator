...
export const PostCard: Component<PostCardProps> = (props) => {
  ...
  const score = () => props.post.upvotes - props.post.downvotes;  return (
    <article class="bg-surface rounded-lg shadow-sm overflow-hidden mb-4">
      <div class="flex">
        {/* Vote Section */}
        <div class="flex flex-col items-center p-4 bg-surface-variant min-w-[60px]">
          <button 
            class="p-2 rounded-full hover:bg-surface-container transition-colors"
            aria-label="Upvote"
          >
            <svg class="w-5 h-5 text-on-surface-variant" viewBox="0 0 24 24" fill="currentColor">
              <path d="M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6z"/>
            </svg>
          </button>
          
          <span class="text-on-surface font-medium py-1">{score()}</span>
          
          <button 
            class="p-2 rounded-full hover:bg-surface-container transition-colors"
            aria-label="Downvote"
          >
            <svg class="w-5 h-5 text-on-surface-variant rotate-180" viewBox="0 0 24 24" fill="currentColor">
              <path d="M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6z"/>
            </svg>
          </button>
        </div>        {/* Content Section */}
        <div class="flex-1 p-4">
          ...
          <div class="flex items-center gap-4 text-on-surface-variant">
            <button class="flex items-center gap-2 px-3 py-2 rounded hover:bg-surface-container transition-colors">
